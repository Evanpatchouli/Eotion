...
2. **Work Unit 难度**：拆解后每个工作单元必须根据自身实际推理需求重新评级，并据此选择 Agent / 模型。

硬规则：

- 父任务的 S2/S3 **不得自动传播**给子任务。
- 非平凡任务拆成少量**可独立验证**的工作单元并记录到 `.agents/current-task.md`；通常 2–6 个即可，不为了数量而拆。
- 每个 Work Unit 只保留一种主要认知模式：调查（investigate）、决策（decide）、执行（execute）、验证（verify/review）。
- **混合 Work Unit 必须优先拆开**：如果一个单元同时包含“需要 Sol 做方案判断”和“方案确定后的大量机械实现/测试/文档”，应在决策边界拆成 S2 -> S1/S0，不能因为放在同一功能里就整段保持 S2。
- 按行为边界、决策边界和验证方式拆分，不按文件数量拆分。
- 拆解时主动暴露可降级工作：代码检索、fixture/测试数据、adapter CRUD、开发验证页、文档、build/typecheck、结果整理通常优先考虑 S0/S1。
- 只有 Work Unit 本身需要普通设计判断、跨模块权衡、接口/invariant 决策时才使用 S2。
- 只有已有真实失败证据且仍存在跨层根因、高风险 invariant 时才使用 S3。
- 调查或决策后任务变简单时必须**重新评级并降级**，不要因为初始判断较难继续占用高成本模型。
- S2/S3 单元应尽量产出紧凑 Implementation Brief：决定、影响文件/接口、invariant、实现步骤、验收、禁止事项、最小验证；后续 S1 只消费 Brief + 最小必要代码上下文。
- 只有相互独立、不会频繁修改同一批文件且并行确有收益的工作才并行。

| 难度 | Work Unit 本身需要什么 | 首选 Agent | 模型 / 强度 |
| --- | --- | --- | --- |
| S0 | 只读定位、读取、总结、执行已知验证、收集证据 | `scout` | GPT-6 Luna / High |
| S1 | 已知方案下的机械实现、fixture、测试脚手架、文档/配置小改 | `fast_worker` | GPT-6 Luna / High |
| S2 | 需要普通设计判断的 feature/bugfix/API/DB/明确边界重构 | `worker` | GPT-6 Sol / Medium |
| S3 | 已有失败证据但根因仍跨层，或并发/事务/安全/高风险迁移 | `deep_solver` | GPT-6 Astra / Low 起步 |
| Review | 非平凡改动独立复核 | `reviewer` | GPT-6 Sol / High |

评级顺序：

1. 只需读、找、运行已知命令或整理证据 -> S0。
2. 已经知道怎么做，只需按明确方案修改代码/测试/文档 -> S1；**代码量大本身不构成 S2**。
3. 仍需在多个合理实现方案中做普通工程判断，或接口/invariant/实现边界尚未决定 -> S2。
4. 已有具体失败证据，但仍需跨抽象层证明根因或处理高风险 invariant -> S3。
5. 如果一个单元同时满足 2 和 3，不按最高等级整包执行；先拆出 S2 决策，再把剩余执行重新评级为 S1/S0。
6. 不确定时先选较低一级，以真实失败证据升级。

升级 / 降级规则：

- Luna 两次仍无法准确定位，或 Work Unit 开始需要跨模块权衡 -> 升级到 S2。
- Sol 已有明确失败证据且仍无法找到根因 -> Sol High 或 S3 / Astra Low。
- Astra 从 Low 起步；只有新的失败证据表明推理仍不足时才升 Medium/High。
- Scout 已把边界查清、后续实现变机械时，应降到 S1，而不是保留 S2/S3。
- Worker 一旦确定接口、invariant 和实现路径，应结束 Sol 的高成本阶段；除非需要最小实现证明方案，否则把后续编码、fixture、测试脚手架、开发页、文档和常规验证交给 S1/S0。
- 不把 Max/Ultra 当默认。
...
- 主 Agent 不应长期亲自执行可以安全交给低成本角色的读重、机械和验证工作。
- **Sol 主 Agent / S2 Worker 默认负责“决定怎么做”，不是在决定之后继续包办全部实现。**
- 但不要为了省模型单价制造无意义的 SubAgent 往返：极短、上下文很少的 S0/S1 小步骤，主 Agent 直接做可能更省。
- 委派时只传递该 Work Unit 所需的最小 Context Pack；S2/S3 决策后的低成本执行优先传 Implementation Brief，不复制整份父任务 Prompt、完整聊天历史或整个仓库上下文。
- 读重任务优先给 `scout`；不要让 Astra 做大范围 grep。
- 核心设计完成后，adapter CRUD、fixture、测试脚手架、开发验证页、文档、配置收尾、build/typecheck/test 汇总优先降到 `fast_worker` / `scout`。
- 已知命令的验证由 `scout` 执行并汇总；只有验证失败需要诊断时再重新评级。
- `deep_solver` 优先用于困难诊断和权衡；诊断后边界清楚时，把实现降级交回 `worker` 或 `fast_worker`。
- 同一事实不要让多个 Agent 重复检索；优先复用已验证的 Evidence Pack。
- 多个写 Agent 不得同时修改同一组文件；存在冲突风险时改为串行。
- 对一个连贯 Roadmap 任务通常只做一次最终独立 review，不默认每个 Work Unit / commit 都重复启动 Sol High Reviewer；高风险边界可做定向 review。
