...
- 非平凡任务先拆成 1–4 个**可独立验证**的工作单元，并记录到 `.agents/current-task.md`。
- 按行为边界和验证方式拆分，不按文件数量拆分。
- 拆解时主动暴露可降级工作：代码检索、fixture/测试数据、机械实现、文档、build/typecheck、结果整理通常优先考虑 S0/S1。
- 只有 Work Unit 本身需要普通设计判断、跨模块权衡或实现边界推理时才使用 S2。
- 只有已有真实失败证据且仍存在跨层根因、高风险 invariant 时才使用 S3。
- 调查后任务变简单时应**降级**，不要因为初始判断较难继续占用高成本模型。
...
2. 已经知道怎么做，只需按明确方案修改少量代码/测试/文档 -> S1。
3. 需要在多个合理实现方案中做普通工程判断 -> S2。
4. 已有具体失败证据，但仍需跨抽象层证明根因或处理高风险 invariant -> S3。
5. 不确定时先选较低一级，以真实失败证据升级。
...
- 核心设计完成后，fixture、文档、配置收尾、build/typecheck/test 汇总优先降到 `fast_worker` / `scout`。
